...
/*To differentiate spi flash used with ecspi controller and qspi controller*/

static int inline is_qspi_supported(struct spi_nor *nor) {
	struct device_node *np = spi_nor_get_flash_node(nor);
	return of_property_read_bool(np, "quad-read");
}

/*
 * issi_quad_enable() - set QE bit in Status Register.
 * @nor:        pointer to a 'struct spi_nor'
 *
 * Set the Quad Enable (QE) bit in the Status Register.
 *
 * bit 6 of the Status Register is the QE bit for ISSI QSPI memories.
 *
 * Return: 0 on success, -errno otherwise.
 */
static int issi_quad_enable(struct spi_nor *nor)
{
	...
	write_sr(nor, val | SR_QUAD_EN_ISSI);
...
static const struct flash_info spi_nor_ids[] = {
	...
	{ "is25wp016",  INFO(0x9d7015, 0, 64 * 1024,  32,
			SECT_4K | SPI_NOR_DUAL_READ | SPI_NOR_QUAD_READ) },
...
#ifndef CONFIG_IWG33M
		params->quad_enable = macronix_quad_enable;
#else
		if(is_qspi_supported(nor))
			params->quad_enable = issi_quad_enable;
		else
			params->quad_enable = NULL;
#endif
		break;
...
{
	nor->params.quad_enable = issi_quad_enable;
	nor->params.set_4byte = NULL;
}
#endif
